// backend/server.js — AttendFlow API Server (MongoDB + Node.js)
...
// Apply query filters to a MongoDB find
...
const server = http.createServer((req, res) => {
  // Apply CORS via middleware pattern
...
const handleRequest = async (req, res) => {
  ...
  try {
    ...
    if (method === "GET" && pathname === "/") {
      return send(res, 200, { status: "ok", app: "AttendFlow API", time: new Date().toISOString() });
    }
    ...
    // GET /collection  (with optional filters + sort)
    if (method === "GET" && !id) {
      const filter = buildFilter(query);
      let cursor   = col.find(filter);
      if (query._sort) {
        cursor = cursor.sort({ [query._sort]: query._order === "desc" ? -1 : 1 });
      }
      let results = await cursor.toArray();
      // Remove _id and passwords
